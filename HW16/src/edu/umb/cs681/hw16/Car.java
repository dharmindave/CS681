...
public class Car {
    ...
    public static void main(String[] args) {
        ...
        int highestprice = noofcars.stream().parallel().map((Car car) ->car.getPrice() ).reduce(0, (ans,price)->ans > price ? ans : price);
        System.out.println("Costliest Price of a car is :"+highestprice);

        int lowestprice = noofcars.stream().parallel().map((Car car) ->car.getPrice() ).reduce(1000000000, (ans, price)->price>ans ? ans : price);
        System.out.println("Cheapest Car Price is :"+lowestprice);

        int averageprice = noofcars.stream().parallel().map((Car car) ->car.getPrice() ).reduce(0, (ans,price) -> ans+price, (finalans, intermediateans) -> finalans)/noofcars.size();
